...
# Put a record into the Kinesis Data Stream
def put_sound_output(data_bytes):
...
def imageFunction(inference_output_q):
...
def soundFunction(inference_output_q):
    ...
    while True:
        ...
        data_bytes = write2stream(part=part, relative_dist=relative_dist, stream=stream)
        # add the sound output in the form of bytes into our kinesis data stream to be received on laptop
        put_sound_output(data_bytes)
...
imageThread = threading.Thread(target=imageFunction, args=(inference_output_q,))
imageThread.start()

# Create and start the second thread to listen for the inference output
soundThread = threading.Thread(target=soundFunction, args=(inference_output_q,))
soundThread.start()
