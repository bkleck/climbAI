...
def imageFunction(inference_output_q, stream):
    ...
    while not stop_event.is_set():
        ...
        output_file = os.path.splitext(vid)[0] + '-result.mp4'
        output_name = os.path.join(test_path, output_file) 
        ...
        prev_part = False # use this to check if previous run had 3-point contact
        prev_hold = False # use this to check if previous run had nearest hold
        ...
        while (cap.isOpened()):
            ...
            if startRun == False:
                ...
                if gotHolds == False:
                    ...
                    start_img = frame.copy()
                    # draw the contours we have obtained from our model with the correct colours
                    for idx, contour in enumerate(contours):
                        cv2.drawContours(start_img, [contour], -1, colour_values[colour_list[idx]], 3)

                    start_img = cv2.cvtColor(start_img, cv2.COLOR_BGR2RGB)
                    cv2.imwrite(os.path.join(test_path, "start-run.jpeg"), start_img[:, :, ::-1])
                    ...
                    for k,v in conditions.items():
                        ...
                        if k == 'lw':
                            ...
                            if start_result != False:
                                ...
                                for body_part in pose_results:
                                    cv2.circle(output_img, pose_results[body_part], 3, (0,255,0), 3)
                        ...
                        if pose_results != None:
                            ...
                            if part != False:
                                ...
                                if type(nearest_hold) != bool:
                                    ...
                                    queue_obj = (part, relative_dist, beep)
...
def soundFunction(inference_output_q, stream):
    ...
    while True:
        part, relative_dist, beep = inference_output_q.get()
        if beep == True:
            print("BEEP")
            output_bytes = beepSound(stream)
        else:
...
imageThread = threading.Thread(target=imageFunction, args=(inference_output_q,stream,))
imageThread.start()

# Create and start the second thread to listen for the inference output
soundThread = threading.Thread(target=soundFunction, args=(inference_output_q,stream,))
soundThread.start()
